import time
import os
from tqdm import tqdm  # 从 tqdm 模块中导入 tqdm 类
    ...
    for attempt in range(10):
        try:
            ...
            if not is_chinese(translated):
                translated = translator.translate(src, dest='zh-cn').text
            return translated
        except Exception as e:
            print(e)
            time.sleep(1)
    return text  # 返回原文作为后备方案

def translate_batch(line):
    ...
    src, trans = parts[0], parts[1]
    if not is_chinese(trans):
        return [src+"\t"+translate_text(src,trans)+"\n"]
    ...
    if filename.endswith("-en.txt"):
        ...
        for i in tqdm(range(0, len(lines))):
            ...
            with open(output_file, 'a', encoding='utf-8') as output_f:
                output_f.writelines(translated_batch)
